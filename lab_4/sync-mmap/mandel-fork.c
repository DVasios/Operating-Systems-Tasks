...
/*
 * The array of the semaphores that are shared among the processes
 */
sem_t *shared_sem;
...
void compute_and_output_mandel_line(int fd, int line, int PROCS)
{
	...
	for(i = 0, curr_line = line; curr_line < y_chars; i++,
		...
		sem_wait(&shared_sem[line]);
	
		output_mandel_line(fd, color_val);
		
		sem_post((line == PROCS -1) ? &shared_sem[0] : &shared_sem[line + 1]);
	}
}
...
{
	if (argc != 2) {
		printf("Usage: ./mandel arg, \n where arg is the number of processes]n");
		return 0;
	}
	...
	// Mapping of the semaphores
	shared_sem = create_shared_memory_area(NPROCS);
	int i;
	

	// Initialization of the semaphores
	for (i = 0; i < NPROCS; i++) {
		sem_init(&shared_sem[i], 1, i==0 ? 1 : 0);
	}
	...
	// Sem destroy
	for (i=0; i < NPROCS; i++) {
		sem_destroy(&shared_sem[i]);
	}

	// Destroy memory area
	destroy_shared_memory_area(shared_sem, NPROCS);
